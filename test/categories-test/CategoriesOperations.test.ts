...
describe('async actions categories', () => {
  ...
  it('If there is a space at the beginning or end of the added incomeCategory, an error message and status code will be printed', async () => {
    const signal = axios.CancelToken.source();
    const addUrl = `${process.env.REACT_APP_ACCOUNT_API_HOST}/categories/custom-categories`;
    ...
    axiosMock.onPost(addUrl, addRequestIncomeCategory).reply(400, errorResponse);

    await addCustomCategories(' キャッシュバック', 1, signal)(store.dispatch);
    expect(store.getActions()).toEqual(expectedAddIncomeCategoryFailActions);
  });

  it('If there is a space at the beginning or end of the added expenseCategory, an error message and status code will be printed', async () => {
    const signal = axios.CancelToken.source();
    const addUrl = `${process.env.REACT_APP_ACCOUNT_API_HOST}/categories/custom-categories`;
    ...
    await addCustomCategories(' ランニングシューズ', 4, signal)(store.dispatch);
  ...
  it('If there is a leading or trailing space when editing a incomeCategory, it will output an error message and status code', async () => {
    const signal = axios.CancelToken.source();
    const id = 14;
    const editUrl = `${process.env.REACT_APP_ACCOUNT_API_HOST}/categories/custom-categories/${id}`;
    ...
    axiosMock.onPut(editUrl, editRequestIncomeCategory).reply(400, errorResponse);

    await editCustomCategories(14, ' 失業手当', 1, signal)(store.dispatch);
    expect(store.getActions()).toEqual(expectedEditIncomeCategoryFailActions);
  });

  it('If there is a leading or trailing space when editing a expenseCategory, it will output an error message and status code', async () => {
    const signal = axios.CancelToken.source();
    const id = 17;
    const editUrl = `${process.env.REACT_APP_ACCOUNT_API_HOST}/categories/custom-categories/${id}`;
    ...
    axiosMock.onPut(editUrl, editRequestExpenseCategory).reply(400, errorResponse);

    await editCustomCategories(17, ' カップラーメン', 2, signal)(store.dispatch);
